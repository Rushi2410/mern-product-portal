...
export default function ProductCardMUI({ product, onEdit, onDelete, showActions = false }) {
  ...
  return (
    <Card variant="outlined" sx={{ height: "100%", display: "flex", flexDirection: "column", backgroundColor: randomColor }}>
      <CardContent sx={{ flexGrow: 1 }}>
        <Typography variant="h6">{product.name}</Typography>
        <Typography variant="body2" color="text.secondary" sx={{ mt: 1 }}>{product.description}</Typography>
      </CardContent>

      <Box sx={{ px: 2, gap:2,pb: 2, display: "flex", justifyContent: "space-between", alignItems: "center" }} >
